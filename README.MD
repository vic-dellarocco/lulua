...
```bash
	MACOSX_DEPLOYMENT_TARGET=10.4 make lulua
```
...
Due to the lack of the posix and lfs modules the following
stdlib.lua functions are not available in Windows:

	ezpath,glob,os.path.isfile

Penlight file and directory functions won't work either.

There are probably many other things that won't work. I haven't
tested Lulua on Windows in years. It is not a priority.
...
interpreter. This file must exist. It can be blank. This means
that there will be no more surprises from errantly set LUA_INIT
variables.
...
* The penlight module is available as pl and penlight.
* The penlight.xml module has been removed. It failed its test
  suite and the fix is not obvious to me.
* penlight.utils.import won't override existing functions so you
  may have to do that yourself. The Lulua stdlib does have many
  of the same function names as penlight (with, of course,
  differing functionality). Namespacing is recommended. I have
  made it easy for you: penlight is available in the penlight
  namespace and the pl namespace.
