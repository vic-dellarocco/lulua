...
function remove_lulua_greeting(s)
    local lines = {}
    for line in s:gmatch("[^\r\n]+") do
        table.insert(lines, line..'\n')
     end

    if #lines > 0 and lines[1]:match("^Lulua Lua Distro") then
        table.remove(lines, 1)
     end

    while #lines > 0 
	and (lines[1]:match("^debugger.lua:")
	or   lines[1]:match("^\27%[33mdebugger.lua: ")) do
        table.remove(lines, 1)
     end
...
do
    ...
    asserteq(stdout, "")
    asserteq(remove_lulua_greeting(stderr), "hello world\n")
    ...
    asserteq(code, 2)
    asserteq(stdout, "")
    asserteq(remove_lulua_greeting(stderr), "hello 42\n")
    ...
    asserteq(code, 2)
    asserteq(stdout, "")
    asserteq(remove_lulua_greeting(stderr), "")
end
...
do
    ...
    asserteq(stdout, "")
    asserteq(remove_lulua_greeting(stderr), "some error\n")
    ...
    asserteq(stdout, "")
    asserteq(remove_lulua_greeting(stderr), "Bad argument expected string; 'default', 'quit', or 'error'. Got 'bad one'\n")
...
do
    -- readlines
    local f = utils.readlines("penlight/tests/test-utils.lua")
